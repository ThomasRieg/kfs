...
void build_initial_user_frame(t_task *t, uint32_t entry, uint32_t user_stack_top, struct process_strings argv, struct process_strings envp)
{
	...
	union { unsigned int n; unsigned char *p; } stck = {.n = user_stack_top };

	unsigned char * *envp0 = (unsigned char **)(stck.n - 16 - argv.string_data.length - envp.string_data.length - 2
		* sizeof(struct elf_auxiliary_vector) - (envp.string_count + 1) * sizeof(char *));
