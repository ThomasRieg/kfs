...
int ext2_write_inode_contents(unsigned int inode_nr, unsigned int offset, const unsigned char *buf, unsigned int buf_size)
{
	...
	if (!ext2_read_inode_struct(fs, inode_nr, &inode))
		return -EBADF;
...
int ext2_mkdir(const char *path, unsigned int mode, unsigned int relative_dir_inode_nr) {
	...
	if (slash && !slash[1]) {
		*slash = 0;
		slash = strrchr(dup_path, '/');
	}
	unsigned int parent_inode_nr;
	char *name;
	if (slash) {
		*slash = 0;
		parent_inode_nr = ext2_path_to_inode(&ext2_mounted, dup_path, relative_dir_inode_nr);
		name = slash + 1;
	} else {
		parent_inode_nr = relative_dir_inode_nr;
		name = dup_path;
	...
	new_entry = (struct ext2_direntry *)((char *)new_entry + new_entry->entry_size);
	new_entry->name_length = 2;
	memcpy(new_entry->name, "..", 2);
	new_entry->entry_size = (sizeof(*new_entry) + new_entry->name_length + 3) & 0xfffffffc;
	new_entry->type_indicator = EXT2_DIR;
...
int ext2_getdirname(unsigned int inode_nr, char *buf, unsigned int size)
{
	...
	*buf = '/';
	...
	while (1)
	{
		...
		if (parent_inode_nr == 0)
			return -EBADF;
		if (parent_inode_nr == inode_nr)
		{
			if (size == written_count)
				return -ERANGE;
			if (written_count > 1 && buf[written_count - 1] == '/')
				buf[written_count - 1] = '\0'; // erase last /
			else
				buf[written_count++] = '\0';
		...
		while ((unsigned char *)direntry < contents.data + contents.length && direntry->entry_size)
		{
			...
			{
				if ((written_count + direntry->name_length + 1) > size)
				{
					VecU8_destruct(&contents);
					return -ERANGE;
				}
				memmove(buf + 2 + direntry->name_length, buf + 1, written_count - 1); // +2 because first / and added /
				memcpy(buf + 1, direntry->name, direntry->name_length);
				memcpy(buf + 1 + direntry->name_length, "/", 1);
				written_count += direntry->name_length + 1;
			}
