...
static void test_panic(const char *msg)
{
    printk("\n[TEST PANIC] %s\n", msg);
    asm volatile("cli");
...
static void test_kmmap_stress(uint32_t iters, uint32_t pages_each, uint32_t flags, const char *name, uint32_t *cs_out)
{
    ...
    for (uint32_t t = 0; t < iters; ++t) {
        ...
        if (!p) TEST_FAILF("kmmap returned NULL ( pages_each=%u flags=%u seed=%p)", pages_each, flags, (void*)seed);
...
static void test_kmmap_fragmentation(uint32_t rounds, uint32_t flags_contig, uint32_t flags_noncontig, uint32_t *cs_out)
{
    ...
    for (uint32_t i = 0; i < HOLD; ++i) { hold[i] = NULL; hold_pages[i] = 0; }
    ...
    for (uint32_t r = 0; r < rounds; ++r) {
        ...
        if (!hold[idx]) {
            ...
            if (!p) TEST_FAILF("kmmap returned NULL ( pages=%u f=%u seed=%p)", pages, f, (void*)seed);
            hold[idx] = p;
    ...
    for (uint32_t i = 0; i < HOLD; ++i)
        if (hold[i]) kmunmap(hold[i], hold_pages[i]);
...
static void test_vmalloc_fragmentation(uint32_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) { ptrs[i] = NULL; sizes[i] = 0; }
    ...
    for (uint32_t i = 0; i < count; i += 2) {
        if (!ptrs[i]) TEST_FAILF("unexpected NULL in ptrs (phase=B i=%u)", i);
    ...
    for (uint32_t i = 1; i < count; i += 2) {
        if (!ptrs[i]) TEST_FAILF("unexpected NULL in ptrs (phase=D i=%u)", i);
...
static void test_vrealloc_stress(uint32_t iters, uint32_t *cs_out)
{
    ...
    for (uint32_t i = 0; i < iters; ++i) {
        ...
        if (!p) TEST_FAILF("vmalloc returned NULL (size=%u seed=%p)", a, (void*)seed);

        for (uint32_t k = 0; k < a; ++k)
            p[k] = (uint8_t)(k ^ 0x5A);

        uint8_t *q = (uint8_t *)vrealloc(p, b);
        if (!q) TEST_FAILF("vrealloc returned NULL (p=%u b=%u seed=%p)", p, b, (void*)seed);

        // must preserve first min(a,b) bytes
        uint32_t m = (a < b) ? a : b;
        for (uint32_t k = 0; k < m; ++k) {
            if (q[k] != (uint8_t)(k ^ 0x5A)) TEST_FAILF("vrealloc didn't copy the first bytes correctly (p=%u b=%u seed=%p)", p, b, (void*)seed);
        }
        ...
        if ((xorshift32(&seed) & 1u) == 0) {
            ...
            if (!r) TEST_FAILF("vrealloc returned NULL (q=%u c=%u seed=%p)", q, c, (void*)seed);
            uint32_t mm = (m < c) ? m : c;
            for (uint32_t k = 0; k < mm; ++k) {
                if (r[k] != (uint8_t)(k ^ 0x5A)) TEST_FAILF("vrealloc didn't copy the first bytes correctly when shrink (q=%u c=%u seed=%p)", q, c, (void*)seed);
            }
            cs ^= vsize(r);
            vfree(r);
        } else {
...
void mem_test_all(void)
{
    ...
    // kmmap: fast repeated
    test_kmmap_stress(1024, 8, PTE_RW | MMAP_CONTIG, "contig", &cs);
    test_kmmap_stress(1024, 8, PTE_RW,              "noncontig", &cs);
