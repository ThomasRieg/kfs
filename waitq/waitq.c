...
/*   Updated: 2026/02/24 16:04:49 by thrieg           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void waitq_wake(t_task *task)
{
	...
	print_trace("wake: pid %u status=%d in_sleep=%d sleep_until=%u next_sleep=%p\n",
				task->task_id, task->status, task->in_sleep_queue,
				task->sleep_until, task->next_sleep_task);
	if (task->sleep_q)
		waitq_remove(task);
