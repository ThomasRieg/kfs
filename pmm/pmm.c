...
static void mark_used_range(uint64_t start_pa, uint64_t end_pa)
{
    ...
    if (first >= g_total_frames) return;
    if (last > g_total_frames) last = g_total_frames;
...
static const t_mb2_tag_mmap *find_mmap_tag(const t_mb2_info *mbi)
{
    ...
    while (p + sizeof(t_mb2_tag) <= end) {
        ...
        if (tag->type == MB2_TAG_MMAP) return (const t_mb2_tag_mmap *)tag;
...
void pmm_free_frame(uint32_t pa)
{
    if ((pa & (PAGE_SIZE - 1u)) != 0) return; // must be aligned
    ...
    if (bit_test(idx)) {
        bit_clear(idx);
        g_free_frames++;
        if (idx < g_first_free) g_first_free = idx;
    }
}
